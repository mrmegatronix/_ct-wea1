...
import { CloudSun, Droplets, Sun, Wind, Eye, Moon } from 'lucide-react';
...
export function CurrentWeatherSlide({ isActive, currTemp }: Props) {
  return (
    <div className={`absolute inset-0 px-4 md:px-16 pb-16 slide-transition flex flex-col justify-center items-center ${isActive ? 'slide-active' : 'slide-exit pointer-events-none opacity-0'}`}>
      <div className="glass-panel w-full max-w-7xl rounded-[2rem] md:rounded-[3rem] p-8 md:p-12 flex flex-col items-center border border-white/10">
        <div className="flex flex-col md:flex-row items-center gap-8 md:gap-16 mb-8">
          <CloudSun className="w-32 h-32 md:w-48 md:h-48 text-primary animate-float" strokeWidth={1} />
        ...
        <div className="text-center mb-10 md:mb-16">
          <h2 className="text-5xl md:text-7xl font-headline-lg font-bold text-on-surface mb-4">Mostly Cloudy</h2>
          <p className="text-3xl md:text-5xl font-body-lg text-on-surface/60 flex items-center justify-center gap-4">
            <span>H: {currTemp + 2}°  L: {currTemp - 6}°</span>
            <span className="text-on-surface/30">|</span>
            <span className="text-clear-blue flex items-center">
              <Droplets className="w-8 h-8 md:w-12 md:h-12 mr-2" strokeWidth={1.5} />15%
            </span>
          </p>
        </div>

        <div className="flex flex-wrap md:flex-nowrap justify-center gap-4 w-full max-w-full">
          <MetricCard icon={<Droplets className="text-secondary-fixed w-8 h-8 md:w-10 md:h-10 mb-2" />} label="Humidity" value="72%" />
          <MetricCard icon={<Sun className="text-sunset-orange w-8 h-8 md:w-10 md:h-10 mb-2" />} label="UV Index" value="3 Mod" />
          <MetricCard icon={<Wind className="text-clear-blue w-8 h-8 md:w-10 md:h-10 mb-2" />} label="Wind" value="24 km/h" />
          <MetricCard icon={<Eye className="text-primary-fixed-dim w-8 h-8 md:w-10 md:h-10 mb-2" />} label="Vis" value="10 km" />
          <MetricCard icon={<Moon className="text-surface-tint w-8 h-8 md:w-10 md:h-10 mb-2" />} label="Moon" value="Waning" />
        </div>
      </div>
    </div>
  );
